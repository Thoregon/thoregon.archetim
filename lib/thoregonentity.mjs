...
*/
    ...
    captureSnapshot(name = 'default') {
        ...
        data.forEach(attribute => {
            // Check if it's in the format 'attribute:functionName()'
            if (attribute.includes(':')) {
                const [key, functionCall] = attribute.split(':');
                const functionName = functionCall.replace('()', '').trim();

                if (typeof this[functionName] === 'function') {
                    snapshot[key] = this[functionName]();
                }
            } else {
                // Nested attributes like 'affiliateProvision.amount'
