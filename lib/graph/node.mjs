...
export default class Node {
     ...
     * @param {String}         [strangeness] key of the STRANGENESS entry
     * @return {Node}
     */
    static root(contenthandler, stangeness) {
    ...
    put(item) {
        if (item.no͛de) {
            ...
            this._contenthandler = item._contenthandler;
...
/*
       return {
           once(fn) {
               Reflect.ownKeys(this._children).filter(name => name !== universe.T && !name.startsWith('_')).forEach(name => fn(name))
           }
       };
*/
    }
