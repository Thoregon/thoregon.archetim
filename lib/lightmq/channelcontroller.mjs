...
import { timeout, doAsync } from "/evolux.universe/lib/thoregonhelper.mjs";
    ...
    async processEntry(evt) {
        ...
        try {
            ...
            try {
                ...
                try { app.current.home.sendStatusEmail('ChannelController 1', 'ERROR', `error in event handler 1: ${evt.type}\n${JSON.stringify(detail, null, 3)}`, error) } catch (eeee) { console.error("ChannelController 3: Could not send status mail", eeee, eeee.stack); }
                console.error('ChannelController 1', error, error.stack);
                let record = true;
                try {
                    if (this.errorhandler) record = !!this.errorhandler({ type, detail, error })
                } catch (ee) {
                    // this will just be logged
                    try { app.home.sendStatusEmail('ChannelController 2', 'ERROR', `error in event error handler 2: ${type}`, ee) } catch (eeee) { console.error("ChannelController 2: Could not send status mail", eeee, eeee.stack); }
                    console.error("** ChannelController 2: Error in error handler:", ee, ee.stack);
                }
                try {
            ...
            try { app.home.sendStatusEmail('ChannelController 3', 'ERROR', `error in event handler 3: ${evt.type}`, e) } catch (eeee) { console.error("ChannelController 3: Could not send status mail", eeee, eeee.stack); }
            console.log("** ChannelController 3: ", e);
        } finally {
            this.idle = true;
        }
    }
