...
export default class ThoregonDecorator extends Reporter(AccessObserver) {
    ...
    constructor(target, parent, { handle$, store, metaClass, encrypt, decrypt, inmem }) {
        super(target, parent);
        this.meta       = { metaClass };
...
/*
    initVarProperties() {
        return new Promise((resolve, reject) => {
            ...
            if (this.varpropsinit) {
                resolve();
                return;
            }
            this.varpropsinit = true;
            let varprops = this.varprops;
            ...
            this.handle$.store.once(async (obj) => {
                await Object.keys(obj).aForEach(async (name) => {
                    if (name === T || name === '_') return;
                    if (encrypted) name = await SEA.decrypt({ ct: name, iv, s: salt }, key);
                    varprops.add(name);
                });
                resolve();
            }).not(() => resolve());
...
*/
    ...
    set(target, prop, value, receiver) {
        ...
        (async () => {
            ...
            // todo [OPEN]: check what kind of value was there before! remove either the simple property of the reference if different
    ...
    deleteProperty(target, prop, receiver) {
        ...
        (async () => {
            const entry = this.handle$.entry;
            const oldValue = Reflect.get(target, prop);       // check if the stored value needs to be loaded first!
            value = this.beforeDelete(target, prop, value, receiver) ?? value;
    ...
    static async __restore__(store, decrypt$) {
        ...
        if (!eentry) throw ErrObjectNotFound();
    ...
    async __storeObjectEntry0__(entry) {
        ...
        enode.put(eentry, /*(ack) => console.log("2 Create", entry, ack)*/);
        await doAsync();
        if (!handle$.listen) {
            enode.on((item, key) => this.modifiedEntry(item, key));       // this is the objects entry
            node.map().on((item, key) => this.modifiedItem(item, key));         // those are the properies with references
            handle$.listen = true;
        }
    }
    ...
    async modifiedEntry(item, key) {
        ...
        if (entry) {
            // check if a 'simple' property was modified
    ...
    async modifiedItem(item, key) {
        ...
        const attrmode = this.metaClass$.variableAttributes;
        let prop;
        if (attrmode === VARIABLE_ATTRIBUTE_MODE.NONE) {
            ...
            if (attrmode == VARIABLE_ATTRIBUTE_MODE.ENCRYPT) {
                const key  = encrypted ? entry.m.k.k : undefined;
                const salt = encrypted ? entry.m.k.s : undefined;
                const iv   = encrypted ? entry.m.k.iv : undefined;
                prop       = await SEA.decrypt({ ct: name, iv, s: salt }, key);
            }
...
/*  lazy init! restore the object with with 'get'
                const obj = await ThoregonObject.from(itemsoul);
                if (obj) {
                    const refsoul = await soul(this.handle$.store.get(ref));
                    this.propsouls[prop] = refsoul;
                    Reflect.set(this.target, prop, obj);
                    // console.log("**> modified item", '[' + prop + ']', item);
                }
*/
                this.emit('change', { prop, obj: this.proxy$ });
            }
    ...
    async __storeReference__(name, obj, meta) {
        ...
        if (!obj.$thoregon) {
            ...
            if (!obj.$thoregonEntity) {
                ...
                obj = ThoregonDecorator.observe(obj, { store: this.handle$.store.get(key), encrypt: this.encrypt$, decrypt: this.decrypt$, inmem: false });
                await obj.__store__();
            } else {
                obj = await obj.create({ store: this.handle$.store.get(key), encrypt: this.encrypt$, decrypt: this.decrypt$, inmem: false });
            }

            /* $@SOUL */

            await doAsync();
            const refsoul = await soul(this.handle$.store.get(key));
            ...
            node.put(ref);
            /* $@SOUL */
            const refsoul = await refsoul(ref);
            this.propsouls[name] = soul;
            /* $@SOUL */
        ...
        this.emit('change', { prop: name, obj: this.proxy$ });
    }
