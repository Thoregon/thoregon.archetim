...
import { isPromise }  from "/evolux.util/lib/objutils.mjs";
...
function is(gunnode) {
    return new Promise(resolve => {
        gunnode
            // .once((res) => { console.log("IS", res); resolve(res != undefined); })
...
*/
    ...
    async __materialize__() {
        ...
        if (!this.handle$) {
            ...
            debuglog("#==> materialize: new soul", root);
        } else {
        ...
        debuglog("#==> materialize: b4 storeObject");
        await this.__storeObject__();
        if (done) {
            debuglog("#==> materialize: done -> send 'materialized'");
    ...
    async __deleteReference__(name, meta) {
        ...
        debuglog("#==> deleteReference", node, name, meta);
        node.put(null);
    }
    ...
    /*async*/ __storeObjectEntry0__(entry) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                debuglog("#==> storeObjectEntry0", node, entry);
    ...
    async __storeReference__(name, obj, meta) {
        ...
        if (!obj.$thoregon) {
            ...
            debuglog("#==> storeReference", node, name, obj, meta);
            node.put(ref);
            /* $@SOUL */
