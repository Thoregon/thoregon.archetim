...
export default class ThoregonDecorator extends Reporter(AccessObserver) {
    ...
    static async from(root, { cls, metaClass, parent, encrypt, decrypt, dothrow } = {}) {
        ...
        if (!isString(root)) {
            store = root;
            root = store._?.["#"] ?? store._?.soul ?? store._?.link;
            if (root == undefined) root = await soul(store);
        } else {
...
*/
    ...
    async __restore__() {
        ...
        let root = store._?.["#"] ?? store._?.soul ?? store._?.link;
        if (root == undefined) root = await soul(store);
        if (root) this.constructor.addKnownEntity(root, proxy);
