...
export default class ThoregonDecorator extends AccessObserver {
            ...
            if (eref != undefined) {
                if (isSerializedRef(eref)) {
                    ...
                    if (!(value?.deleted)) {
                            ...
                            value?.__checkDeepListeners__?.(receiver, prop);        // todo [OPEN]: only when there are deep listeners
                    } else {
    ...
    __set__(target, prop, value, receiver) {
        ...
        if (this.hasDeepListeners()) value?.__checkDeepListeners__?.(receiver, prop);       // todo [OPEN]: only when there are deep listeners
    }
