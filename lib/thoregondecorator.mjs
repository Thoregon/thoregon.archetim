    ...
    async reservePropertyStore(name, Cls) {
        ...
        if (varAttrs === ATTRIBUTE_MODE.NAMED) {
            ...
            if (!propertiesmap[prop]) {
                ...
                Reflect.set(this.target, name, reserved, this);
                return reserved;
            }
            ...
            Object.entries(simpleprops).forEach(([prop, newValue]) => {
                ...
                if (newValue !== oldValue) {
                    ...
                    if (newValue == undefined) {
                        ...
                        Reflect.set(this.target, prop, newValue, this);
                        if (shouldEmit(prop)) mods.push(prop);
                    }
                }
            })

            if (!dels.is_empty || !mods.is_empty) {
                this.emit('change', { property: '*', obj: this.proxy$, type: 'set' });
                this.metaClass$?.emitEntityEvents(this.proxy$);
            }
        }
    }
...
*/
        ...
        Reflect.set(this.target, name, obj, this); // replace the object in the property with the wrapper (proxy)
