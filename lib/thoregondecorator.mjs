...
// const elems     = (path) => path.toString().split('.');
// const pathparts = (path) => { let items = elems(path); let next = items.shift(); return [next, items]; };
...
/*
    async prop(path, value) {
        ...
        if (value != undefined) {
            ...
            const setprop = parts.pop();
            const sub = await this._prop0(path);
            if (sub == undefined) return;
            await sub.setProp(this.target, setprop, value, this.proxy$);
            return await sub._prop0(setprop);
        } else {
            return await this._prop0(path);
        }
    }
    ...
    async _prop0(path) {
        if (!path) return;
        let parts = Array.isArray(path) ? path : pathparts(path);
        let sub = await this.get(this.target, parts[0], this.proxy$);
        return sub == undefined ? undefined : (parts[1].length > 0) ? await sub._prop0([parts[1].shift(), parts[1]]): sub;
    }
*/
    ...
    async __materialize__() {
        ...
        if (await ThoregonDecorator.materialized(this.handle$.store)) console.log("entity overwrite"); // throw ErrObjectOverwrite();
    ...
    doGet(target, prop, receiver) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (Reflect.get(target, prop) == undefined) {
    ...
    set(target, prop, value, receiver) {
        this.setProp(target, prop, value, receiver);       // yes, this must run async. If you need to
        return true;
    }
    ...
    async setProp(target, prop, value, receiver) {
        ...
        if (!propertySpec.persistent) return; // this attribute can not be persistent and therefore is also rejected
        ...
        try {
            ...
            if (isNil(value)) {
                ...
                this.emit('change', { property: prop, obj: this.proxy$ });
...
*/
            ...
            Object.entries(simpleprops).forEach(([prop, newValue]) => {
                ...
                if (newValue !== oldValue) {
                    ...
                    if (newValue == undefined) {
                        ...
                        this.emit('change', { property: prop, obj: this.proxy$ });    // other event type?
                    } else {
    ...
    async __storeReference__(name, obj, meta) {
        ...
        if (modified) this.emit('change', { property: name, obj: this.proxy$ });
    }
