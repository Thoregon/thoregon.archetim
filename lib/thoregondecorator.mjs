...
export default class ThoregonDecorator extends AccessObserver {
     ...
     */
    static observe(target, { props, parent, soul, handle, create, Cls, metaClass, encrypt, decrypt, amdoc, withDeleted, tombstone, incommingSync = false, load = false, readonly = false } = {}) {
        if (target == undefined) return undefined;
        const proxy = super.observe(target, { props, parent, soul, handle, create, Cls, metaClass, encrypt, decrypt, amdoc, withDeleted, tombstone, load, readonly });
    ...
    static from(soul, { Cls, metaClass, encrypt, decrypt, withDelete, tombstone, incommingSync = false } = {}) {
        let entity = this.getKnownEntity(soul);
        if (entity) return entity;

        entity = this.__restore__(soul, { Cls, metaClass, encrypt, decrypt, withDelete, tombstone, incommingSync });
        if (this.inTX) {
            entity.involve$();
        }
        return entity;
...
*/
    ...
    pop(...args) {
        ...
        const amdoc = this.amdoc;
        this.amdoc = AM().change(amdoc, (doc) => this.__syncEntity2AM__(this.target, doc, true));
...
*/
    ...
    static __restore__(soul, { Cls, metaClass, encrypt, decrypt, withDelete, tombstone, incommingSync = false } = {}) {
        ...
        const target = (amdoc) ? this.__entityFrom__(amdoc, Cls) : Cls ? new Cls() : {};
        const handle = amdoc?._handle;
        const proxy = this.observe(target, { soul, handle, Cls, metaClass, encrypt, decrypt, amdoc, withDelete, tombstone, incommingSync, load:true });
        return proxy;
    }
    ...
    __syncEntity2AM__(from, to, clean = false) {
        if (!to._) to._ = {};
