...
export default class ThoregonDecorator extends AccessObserver {
    ...
    __syncAM2Entity__(from, to, curr) {
        ...
        Object.entries(from).forEach(([prop, value]) => {
            ...
            if (isNil(value)) {
                ...
                if (toval || currval) this.mergeReferences(prop, to, currval, toval, value);
    ...
    mergeReferences(prop, to, currref, currval, otherref) {
        ...
        this.__setAMProperty__(prop, other);
        if (!other.materialized) {
            this.dolog("mergeReferences need sync other", prop, currref, otherref);
            other.addEventListener('synced', (evt) => other.merge(currval, prop), { once: true } );
        } else {
            other.merge(currval, prop);
        }
        // this.__setAMProperty__(prop, otherref); // this was done by the sync already

        // old entity may be removed from DB (GC -> must check if its referenced somewhere)
        SYNC().dropResource(currref);
    }

    merge(other, prop) {
        this.__merge__(other, prop);
    }

    __merge__(other, prop) {
