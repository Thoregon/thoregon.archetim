...
export default class ThoregonDecorator extends AccessObserver {
     ...
     */
    static observe(target, { props, parent, soul, handle, create, Cls, metaClass, encrypt, decrypt, amdoc, withDeleted, tombstone, load = false, readonly = false } = {}) {
...
*/
    ...
    isEnumerable(name) {
        ...
        if (!Array.isArray(this.target) && name === 'length') return false;
...
*/
    ...
    doGet(target, prop, receiver, opt = {}) {
        ...
        if (prop === 'length') {
            // todo [REFACTOR]: if nedded add other 'functional' properties, extract to method
            return this.$keys.length; // Object.keys(this.target).length;
        }
        if (prop === 'soul') {
            // todo [REFACTOR]: if nedded add other 'functional' properties, extract to method
            return this._soul;
    ...
    static __restore__(soul, { Cls, metaClass, encrypt, decrypt, withDelete, tombstone } = {}) {
        ...
        const proxy = this.observe(target, { soul, handle, Cls, metaClass, encrypt, decrypt, amdoc, withDelete, tombstone, load:true });
        return proxy;
    }
    ...
    __syncAM2Entity__(amfrom, totarget, amremote) {
...
*/
    ...
    chooseOrigin(curramdoc, originremote, originlocal) {
        let amdoc = curramdoc;
        if (originlocal.startsWith('builtin:') && originremote.startsWith('repo:')) amdoc = AM().change(amdoc, (doc) => doc._.origin = originremote);
        if (amdoc._.origin.startsWith('builtin:')) {
