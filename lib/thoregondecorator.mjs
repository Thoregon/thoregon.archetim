...
export default class ThoregonDecorator extends AccessObserver {
    ...
    constructor(target, { parent, soul, handle, Cls, metaClass, encrypt, decrypt, is, amdoc } = {}) {
        super(target, parent);
     ...
     */
    static /*async*/ available(soul, { Cls, metaClass, encrypt, decrypt } = {}) {
        return new Promise((resolve, reject) => {
            let entity = this.from(soul, { Cls, metaClass, encrypt, decrypt } = {});
     ...
     */
    delete() {
        this.amdoc._.del = true;
        this.__materialize__();
        this.emit('delete', { obj: this.proxy$, type: 'delete' });
        this.__requestSync__();
    }

    get deleted() {
        return !!this.amdoc._?.del;
    }
    ...
    ownKeys(target) {
        ...
        return [...keys].filter((prop) => this.isEnumerable(prop)/* && !isNil(Reflect.get(target, prop, this.proxy$))*/);
    }
        ...
        if (value == undefined) {
            ...
            if (eref != undefined) {
                if (isSerializedRef(eref)) {
                    ...
                    //    value.addEventListener('delete', (evt) => this.__propertyDeleted__(prop, value, evt));
                    // }
                } else {
                    if (prop in this.amdoc) {
                        value = this.amdoc[prop];
                        this.__set__(target, prop, value, receiver);
                    }
                }
            } else {
    ...
    __set__(target, prop, value, receiver) {
        ...
        if (this.hasDeepListeners()) value?.__checkDeepListeners__?.(receiver, prop);
    }

    __propertyDeleted__(prop, entity, evt) {
        if (!isPrivateProperty(prop) && !this.dontEmit(prop)) this.emit('change', { obj: this.proxy$, property: prop, newValue: undefined, oldValue: entity });
        entity.removeAllListeners('delete');
    }
    ...
    __syncAM2Entity__(amfrom, totarget, initial = false) {
        ...
        Object.entries(amfrom).forEach(([prop, value]) => {
            ...
            if (isNil(value)) {
                ...
                // just use all other values.
                this.__set__(totarget, prop, value);
...
*/
    ...
    __merge__(parenthandler, to, other, prop) {
        ...
        parenthandler.__set__(to, prop, this.proxy$);
        parenthandler.__setAMProperty__(prop, this.proxy$);
