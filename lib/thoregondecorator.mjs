...
*/
    ...
    isEnumerable(name) {
        if (typeof name === 'symbol' || isPrivateProperty(name) || nonEnumerable(name)) return false;     // no symbols are emumerable
    ...
    __initEmbedded__() {
        ...
        const embeddedAttrs = Object.entries(attributes).find(([name, attrSpec]) => !isPrivateProperty(name) && attrSpec.embedded && !(attrSpec.emergent));
        const embeddedNames = embeddedAttrs.map(([name, attrSpec]) => name);
        ...
        Object.entries(amfrom).forEach(([prop, value]) => {
            ...
            if (isNil(value)) {
                ...
                if (currval) {
                    ...
                    Reflect.get(target, prop, this.$proxy);
                }
