...
*/
    ...
    __initLazyProps__() {
        // const current = new Set(Object.keys(this.target));
    ...
    __mergeEntities__(othervalue, value, prop, parent, receiver) {
        ...
        } else if (othervalue.constructor === Object || value.constructor === Array) {
    ...
    doSet(target, prop, value, receiver, opt = {}) {
        ...
        const oldvalue =  Reflect.get(receiver ?? target, prop); // Reflect.get(target, prop, receiver);
        ...
        if (modified && propertySpec.storeIt) this.__setAMProperty__(prop, value);
        if (this.materialized) {
            this.__materialize__();
            if (value != undefined && this.materialized) value.materialize?.();
        }
        if (modified) this.__maintainTimestamps__();
        return modified;
    }
