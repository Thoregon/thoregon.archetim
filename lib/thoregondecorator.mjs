...
export default class ThoregonDecorator extends Reporter(AccessObserver) {
    ...
    constructor(target, parent, { handle$, store, metaClass, encrypt, decrypt, inmem }) {
        ...
        if (handle$) {
            ...
            this.handle$ =  isString(store) ? { where: store, store: universe.archetim.persistenceRoot[store] } : { where: store.soul, store };
        }
    ...
    static async from(root, { cls, metaClass, parent, encrypt, decrypt }) {
        ...
        if (await root.is) {
            ...
            store =  universe.archetim.persistenceRoot[root];
...
*/
    ...
    set(target, prop, value, receiver) {
        ...
        */
        ...
        // if (!value.$thoregon) value = ThoregonDecorator.observe(value); // todo: create a reuseable proxy
        // super.doSet(target, prop, (simpleSerialize(value) || propertySpec.embedded || value.$thoregon) ? value : ThoregonDecorator.observe(value), receiver);     // will be replaced with the proxy after it is stored
    ...
    async __store__() {
        if (!this.handle$) {
            ...
            this.handle$ = { where: root, store: universe.archetim.persistenceRoot[root] };
        }
    ...
    async __storeReference__(name, obj, meta) {
        ...
        if (!obj.$thoregon) {
            ...
            if (!obj.$thoregonEntity) {
                ...
                obj = ThoregonDecorator.observe(obj, { store: this.handle$.store[key], encrypt: this.encrypt$, decrypt: this.decrypt$, inmem: false });
                await obj.__store__();
            } else {
                obj = await obj.create({ store: this.handle$.store[key], encrypt: this.encrypt$, decrypt: this.decrypt$, inmem: false });
            }
