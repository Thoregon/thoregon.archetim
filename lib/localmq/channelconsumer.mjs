...
export default class ChannelConsumer {
    ...
    _connectService() {
        (async () => {
            ...
            try {
                this.service = await universe.mq.getService(this.terminalid);
                this._processQ();
            } catch (e) {
                setTimeout(() => this._connectService(), RETRY_INTERVAL);
            }
        })();
    }
    ...
    sendEvent(type, detail = {}) {
        ...
        if (!this.service?.isReady()) return this._enqueue(type, detail);    // enqueue when service is not ready
        this._sendEvent(type, detail);
    }

    async _sendEvent(type, detail) {
        try {
            await this.service.sendEvent(this.name, type, detail);
        } catch (e) {
